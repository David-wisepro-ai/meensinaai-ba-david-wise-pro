...
- **Ligações no CRM (FUNCIONANDO desde 2026-06-27):** o CRM liga pros leads por **telefone direto (PSTN, NÃO via WhatsApp)**, gravado. Número de saída = **+1 (774) 737-7996** (número LC comprado dentro do GHL, Milford/MA, Local, ~US$ 1,15/mês + por minuto), cadastrado como **Default Number** ("David's number"), com **Call Recording ON** + aviso pt-BR ("Esta ligação pode ser gravada para garantir a qualidade do atendimento"). Testado 2026-06-27: chamada conectou e gravou. David/Alessandra clicam "Ligar" no lead → sai do 774-737-7996, gravada.
  - **Como destravou:** a Twilio exige **verificação de identidade** (Persona: foto do documento + selfie) pra qualquer ligação de saída pra outros números. O botão "Verify ID" da tela de chamada estava **quebrado** (abria página em branco), e a ligação por WhatsApp dava "Not Eligible" (coexistência + Meta não verificado). O fluxo que FUNCIONOU foi: comprar um número (Add Phone Number) → no checkout o GHL dispara a verificação Persona certinha → David fez selfie + CNH → liberou TODAS as ligações de saída da conta. David completou a verificação em 2026-06-27.
  - **Também existe** o Verified Caller ID **+1 (774) 777-8510** (o número do WhatsApp, mascarado) cadastrado, mas o número de ligação padrão agora é o 774-737-7996.
  - **A2P 10DLC** segue incompleto/pendente — mas é **só pra mandar SMS** desse número, NÃO afeta ligação. Fazer só se quiser textar leads por SMS (registro de empresa, 1-3 dias).
  - **Recebimento:** o 774-737-7996 também recebe ligação de volta dentro do CRM (é número LC de verdade). MA é estado de consentimento de todas as partes pra gravação → por isso o aviso no início.
...
- **Acessos reais ao Portal Project Manager (matrículas criadas 2026-06-27):** `alefreirecopywriter@gmail.com` (Alessandra, vendedora) e `david.dazilio@hotmail.com` (David) — ambos com enrollment `project_manager` no Supabase. Cada um cria a própria senha via signup do portal.
- **Login do portal (Supabase Auth):** "Confirm email" está DESLIGADO (signup é instantâneo, sem email de confirmação). O **Site URL** estava errado (`http://localhost:3000`) e QUEBRAVA magic link / reset de senha pra todo mundo — **corrigido em 2026-06-27 pra `https://wiseproacademy.vercel.app`**. Fluxo pra dar acesso a um aluno: (1) inserir linha em `enrollments` (email minúsculo, product `project_manager` ou `construtor`) no Supabase; (2) o aluno faz **signup** no portal com o mesmo email → entra na hora. Se esquecer a senha: no Supabase Dashboard → Authentication → Users → achar o email → "Send magic link" (email entrega OK).
- **Turma Construtor (CSL) — alunos liberados:** (2026-06-27) `cristinaparty@outlook.com`, `joaodelala@hotmail.com`, `corporationpowerup@gmail.com`; (2026-08-07) `fernandogriebler@gmail.com` — matrícula `construtor` + login, senha comum `WisePro2026!`. Abre o Portal Construtor (simulados CSL). Testado via API GoTrue = OK. ATENÇÃO: joaodelala e corporationpowerup já tinham conta com senha própria; o insert de auth NÃO troca senha de conta existente — foi preciso rodar UPDATE encrypted_password depois pra padronizar. **Sempre rodar o UPDATE de senha + testar login por API depois de criar em lote.**
- **Turma Project Manager liberada (2026-06-27):** 16 alunos com acesso ao Portal PM (login + enrollment `project_manager`), **senha comum `WisePro2026!`** pra todos. A turma de Construtor ainda NÃO foi aberta. Emails: paulorlandi.ssouza / celiocosta2009 / franklin.oliveira.idf / johnnathan.almeida / drumond.kleber / ggsf87 / silvapereiraddd / jardelsoliveira2000 / lrssalmeida / ricardoconte91 / leo.per1993 / rambomg80 / filipe_1608 / robertfreitas156 / david.dazilio / alefreirecopywriter. Logins criados via SQL direto no Supabase (auth.users + auth.identities clonando estrutura de conta funcional; provider_id = user_id, identity_data com sub+email+email_verified). Testado via API GoTrue (grant_type=password) = OK. **Pra criar login por SQL:** email minúsculo, encrypted_password = crypt(senha, gen_salt('bf')), email_confirmed_at = now(), + linha em auth.identities. ⚠️ **Colunas de token da `auth.users` (confirmation_token, recovery_token, email_change, email_change_token_new, email_change_token_current, phone_change, phone_change_token, reauthentication_token) DEVEM ser string vazia `''`, NUNCA NULL** — se ficarem NULL o login dá **"Database error querying schema"** (o GoTrue faz Scan de string e NULL quebra). Setar via `COALESCE(col,'')` num DO block que só toca as colunas que existirem (ver `scratchpad/aluno_fix.sql`, 2026-08-07 caso Fernando). **Regra: sempre VERIFICAR com SELECT depois** (a 1ª vez que o David rodou um insert de enrollment "deu certo" mas não inseriu nada).
- **Aulas do Portal PM (frontend/components/PortalLessons.tsx, array PM_AULAS):** 1 Funções do PM · 2 Sequência da Obra · 3 OSHA · 4 Leituras de Plantas para Orçamentos e Change Orders · 5 IA para Gestão de Projetos na Construção. Aulas 6-8 vazias. Vídeos 4 e 5 no Google Drive (provisório). Construtor: todas as 8 aulas ainda vazias.
